...
class magasinsCategories
{
    ...
    {
        $contact_id = wa('magasins')->getUser()->getId();
    ...
    public static function clear($category_id = null, $recurse=false)
    {
        ...
        if ($category_id === null) {
            ...
            self::getSettingsModel()->delete($contact_id, 'magasins', self::$natural_mode_key);
        } else {
    ...
    private function getTree($parent_id = 0)
    {
        ...
        $category_routes_model = new magasinsCategoryRoutesModel();
    ...
    private static function getCategoryModel()
    {
        if (!self::$category_model) {
            self::$category_model = new magasinsCategoryModel();
        }
    ...
    public static function getCategoryExpandedStatesMap()
    {
        ...
        $natural_mode = false;
        $settings = array();
        foreach ($settings_model->get($contact_id, 'magasins') as $name => $item) {
